...
resource "azuread_application_federated_identity_credential" "gha_tfdeploy_main" {
  ...
  display_name   = "gha-${local.deploy_github_repo}-${replace(local.deploy_github_ref, "/", "-")}"
  issuer         = "https://token.actions.githubusercontent.com"
  subject        = "repo:${local.deploy_github_org}/${local.deploy_github_repo}:ref:${local.deploy_github_ref}"
